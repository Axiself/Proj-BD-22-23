...
AS
SELECT a.SKU, a.order_no, a.qty, (a.qty*d.price) AS total_price, EXTRACT(YEAR FROM c.date) AS year, EXTRACT(MONTH FROM c.date) AS month, EXTRACT(DAY FROM c.date) AS day, EXTRACT(DOW FROM c.date) AS day_of_the_week
FROM contains a
